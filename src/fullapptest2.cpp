...
void displayRemainingOrders(const priority_queue<Order, vector<Order>, BuyComparator>& buyQueue,
                            const priority_queue<Order, vector<Order>, SellComparator>& sellQueue) {
    priority_queue<Order, vector<Order>, BuyComparator> buyCopy = buyQueue;
    priority_queue<Order, vector<Order>, SellComparator> sellCopy = sellQueue;

    cout << "Remaining Buy Orders:" << endl;
    while (!buyCopy.empty()) {
        const Order& order = buyCopy.top();
        cout << "Order ID: " << order.orderID << "\tInstrument: " << order.instrument
             << "\tSide: " << order.side << "\tPrice: " << order.price
             << "\tQuantity: " << order.quantity << endl;
        buyCopy.pop();
    }

    cout << "Remaining Sell Orders:" << endl;
...
int main() {
    ...
    for (const Order& order : orders) {#include <iostream>
#include <fstream>
#include <sstream>
#include <vector>
#include <queue>
...
int main() {
    ...
    displayRemainingOrders(buyQueue, sellQueue);
